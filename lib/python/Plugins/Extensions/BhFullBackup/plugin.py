...
class Bh_Bfull_main(Screen, ConfigListScreen):
    ...
    def __init__(self, session):
        ...
        self['lab1'] = Label('')
        self['actions'] = ActionMap(['WizardActions', 'ColorActions'], {'red': self.doBackUp,
         'green': self.close,
         'back': self.close})
        self.updateList()
        self.deviceok = True
    ...
    def updateList(self):
        ...
        if myusb1:
            myoptions.append((myusb1, myusb1))
        if myhdd:
            myoptions.append((myhdd, myhdd))
        self.list = []
        self.myepg_path = NoSave(ConfigSelection(choices=myoptions))
        epg_path = getConfigListEntry(_('Path to store Full Backup'), self.myepg_path)
        if len(myoptions) > 0:
    ...
    def doBackUp(self):
        if self.myepg_path.value:
            mytitle = _('Black Hole Full Backup on: ') + self.myepg_path.value
            cmd = '/usr/bin/bh_backup_full.sh ' + self.myepg_path.value
...
def menu(menuid, **kwargs):
    if menuid == 'bhbackup':
        return [(_('BlackHole Full Backup'),
          main,
          'BlackHoleFullBackup',
          1)]
    return []
